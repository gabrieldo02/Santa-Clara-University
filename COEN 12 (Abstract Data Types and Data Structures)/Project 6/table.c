...
{
    void *pivot=elts[high]; // select pivot
    void *temp;
    int i = low;
    int j;
    for(j=low;j<high;j++)
    {
        if((*compare)(elts[j],pivot)<0) // if less than pivot
        {
            temp=elts[j];
            elts[j]=elts[i];
            elts[i]=temp; // swap elements
            i++; // increment
        }
    }
    temp=elts[high]; // when pivot is smaller
    elts[high]=elts[i];
    elts[i]=temp; // swap to find position of pivot
    return i; // pivot location 
}

/* Big-O Notation: O(nlogn) ideal; O(n^2) worst */
static void quicksort(void **elts, int low, int high, int (*compare)())
{
    int pIdx;
    if(low<high)
    {
        pIdx = partition(elts,low, high, compare); // index of pivot
        quicksort(elts,low,pIdx-1, compare); // reorder elements before pivot
        quicksort(elts,pIdx+1,high,compare); // reorder elements after pivot
    }
}
...
void *getElements(SET *sp) // allocate and return a sorted array of elements in the set pointed to by sp
{
    ...
    quicksort(elts, 0, sp->count-1,sp->compare); // quicksort algorithm
    return elts;
}
